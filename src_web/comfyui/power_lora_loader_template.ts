...
class RgthreePowerLoraLoaderTemplate extends RgthreeBaseServerNode {
    constructor(title = NODE_CLASS.title) {
        super(title);
        this.serialize_widgets = true;
        this.logger = rgthree.newLogSession(`[Power Lora Stack Template]`);
                        ...
                        rgthreeApi.getPowerLoraTemplates(value).then((resp) => {
                            ...
                            for (const w of current) {
                                ...
                                if ((_b = w.name) === null || _b === void 0 ? void 0 : _b.startsWith("lora_")) {
                                    removeArrayItem(this.widgets, w);
                                }
                            }
...
RgthreePowerLoraLoaderTemplate.title = NodeTypesString.POWER_LORA_LOADER_TEMPLATE;
RgthreePowerLoraLoaderTemplate.type = NodeTypesString.POWER_LORA_LOADER_TEMPLATE;
RgthreePowerLoraLoaderTemplate.comfyClass = NodeTypesString.POWER_LORA_LOADER_TEMPLATE;
RgthreePowerLoraLoaderTemplate[_a] = {
    type: "combo",
...
const NODE_CLASS = RgthreePowerLoraLoaderTemplate;
app.registerExtension({
    name: "rgthree.PowerLoraLoaderTemplate",
